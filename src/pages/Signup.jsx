    ...
    <div style={s.wrap}>
      <div style={s.card}>
        ...
        <div style={{display:'flex',gap:6,marginBottom:24}}>
          {[1,2].map(n => (
            <div key={n} style={{height:3,flex:1,borderRadius:2,background: n <= step ? '#0f172a' : '#e2e8f0',transition:'background .2s'}} />
          ))}
        </div>
          ...
          <>
            ...
            {trades.length < 10 && (
              <div style={{position:'relative',marginBottom:12}}>
                <input
                  ...
                  style={{...s.input, paddingRight:tradeInput?40:14, borderColor: showDrop && tradeInput ? '#0f172a' : '#e2e8f0'}}
                />
                {tradeInput && (
                  <button onMouseDown={e=>{e.preventDefault();setTradeInput('');setShowDrop(false)}}
                    style={{position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',color:'#94a3b8',cursor:'pointer',fontSize:18,lineHeight:1}}>×</button>
                )}
                {showDrop && tradeInput.trim() && (suggestions.length > 0 || showExact) && (
                  <div style={{position:'absolute',top:'100%',left:0,right:0,background:'#fff',border:'1px solid #e2e8f0',borderTop:'none',borderRadius:'0 0 10px 10px',zIndex:100,boxShadow:'0 8px 16px rgba(15,23,42,.08)',maxHeight:220,overflowY:'auto'}}>
                    {suggestions.map((sg, i) => (
                      <button key={sg} onMouseDown={()=>addTrade(sg)}
                        style={{width:'100%',background:'none',border:'none',borderTop:i>0?'1px solid #f1f5f9':'none',padding:'10px 14px',textAlign:'left',color:'#334155',fontSize:13,cursor:'pointer'}}>
                        {sg}
                      </button>
                    ))}
                    {showExact && (
                      <button onMouseDown={()=>addTrade(tradeInput)}
                        style={{width:'100%',background:'#f8fafc',border:'none',borderTop:'1px solid #e2e8f0',padding:'10px 14px',textAlign:'left',color:'#0f172a',fontSize:13,fontWeight:600,cursor:'pointer'}}>
            ...
            {trades.length === 0 && !tradeInput && (
              <div style={{marginBottom:16}}>
                <div style={{fontSize:11,fontWeight:600,color:'#94a3b8',letterSpacing:'1px',textTransform:'uppercase',marginBottom:8}}>Populaires</div>
                <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                  {TRADE_EXAMPLES.map(t => (
                    <button key={t} onClick={()=>addTrade(t)}
                      style={{background:'#f8fafc',border:'1px solid #e2e8f0',color:'#475569',borderRadius:18,padding:'6px 12px',fontSize:12,cursor:'pointer'}}>
                      + {t}
                    </button>
                  ))}
                </div>
            ...
            <button type="button" onClick={submit} disabled={loading}
              style={{width:'100%',padding:'10px',border:'none',background:'none',color:'#94a3b8',fontSize:13,cursor:loading?'not-allowed':'pointer',marginTop:8}}>
              Passer cette étape
            </button>
          </>
        )}
