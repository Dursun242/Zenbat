        ...
        <div key={l.id} style={{ padding: "10px 12px", borderBottom: "1px solid #f8fafc", background: l.type_ligne === "lot" ? "#f8fafc" : "white" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
            ...
            <button onClick={() => move(l.id, -1)} disabled={idx === 0}
              style={{ background: "none", border: "none", color: idx === 0 ? "#cbd5e1" : "#64748b", cursor: idx === 0 ? "default" : "pointer", fontSize: 13, padding: "2px 6px" }}>↑</button>
            <button onClick={() => move(l.id, 1)} disabled={idx === lignes.length - 1}
              style={{ background: "none", border: "none", color: idx === lignes.length - 1 ? "#cbd5e1" : "#64748b", cursor: idx === lignes.length - 1 ? "default" : "pointer", fontSize: 13, padding: "2px 6px" }}>↓</button>
          ...
          {l.type_ligne === "lot" ? (
            <input
              ...
              style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 10px", fontSize: 12, fontWeight: 700, letterSpacing: .3, color: "#0f172a", background: "white", boxSizing: "border-box" }}
            />
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <input
                ...
                style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 10px", fontSize: 12, color: "#0f172a", boxSizing: "border-box" }}
              />
              <input
                value={l.lot || ""}
                onChange={e => update(l.id, { lot: e.target.value })}
                placeholder="Lot (ex. Plomberie)"
                style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 8, padding: "6px 10px", fontSize: 11, color: "#475569", boxSizing: "border-box" }}
              />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6 }}>
                <div>
                  ...
                  <input type="number" inputMode="decimal" step="0.01"
                    ...
                    style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 8, padding: "6px 8px", fontSize: 12, color: "#0f172a", boxSizing: "border-box" }}/>
                </div>
                <div>
