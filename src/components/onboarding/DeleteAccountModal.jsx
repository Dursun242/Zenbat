  ...
  return (
    <div onClick={() => !busy && onClose()}
      ...
      <div onClick={e => e.stopPropagation()}
        style={{ background: "#1A1612", border: "1px solid #3D3028", borderRadius: 18, maxWidth: 440, width: "100%", padding: 22 }}>
        ...
        <div style={{ fontSize: 12, color: "#9A8E82", lineHeight: 1.6, marginBottom: 14 }}>
        ...
        <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#9A8E82", marginBottom: 6 }}>
          Saisissez votre email <strong style={{ color: "white" }}>{myEmail || "—"}</strong> pour confirmer :
        </label>
        <input value={confirm} onChange={e => setConfirm(e.target.value)}
          placeholder={myEmail}
          disabled={busy} autoFocus
          style={{ width: "100%", background: "#2A231C", border: "1px solid #3D3028", borderRadius: 10, padding: "10px 12px", fontSize: 13, color: "white", outline: "none", marginBottom: 12, fontFamily: "inherit" }}/>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onClose} disabled={busy}
            style={{ flex: 1, background: "#2A231C", color: "#9A8E82", border: "none", borderRadius: 10, padding: "11px", fontSize: 12, fontWeight: 700, cursor: busy ? "not-allowed" : "pointer" }}>
            Annuler
          </button>
