...
export default function AdminNegativeLogs({ iaNegs, loading, negFilter, setNegFilter, onRefresh }) {
  ...
  return (
    <div style={{ background: "white", borderRadius: 14, overflow: "hidden", marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,.06)" }}>
      ...
      {iaNegs === null && !loading && (
        <div style={{ padding: 16, textAlign: "center", fontSize: 11, color: "#9A8E82" }}>Appliquez la migration 0005 pour activer ce journal.</div>
      )}
      {iaNegs && filtered.length === 0 && (
        <div style={{ padding: 16, textAlign: "center", fontSize: 11, color: "#9A8E82" }}>Aucun signal négatif sur cette période 🎉</div>
      )}
      {iaNegs && filtered.slice(0, 50).map((n, i) => {
        const isRefusal = n.kind === "ai_refusal";
        const bg  = isRefusal ? "#F0EBE3" : "#fef2f2";
        const fg  = isRefusal ? "#6B6358" : "#b91c1c";
        const tag = isRefusal ? "Refus IA" : "Usager mécontent";
        return (
          <div key={n.id} style={{ padding: "10px 16px", borderBottom: "1px solid #FAF7F2", background: i % 2 === 0 ? "white" : "#fafbfc" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
              <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 20, background: bg, color: fg }}>{tag}</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: "#1A1612", overflow: "hidden", textOverflow: "ellipsis" }}>{n.name}</span>
              <span style={{ fontSize: 9, color: "#9A8E82" }}>{n.email}</span>
              <span style={{ marginLeft: "auto", fontSize: 9, color: "#9A8E82" }}>{fmtDT(n.created_at)} · {new Date(n.created_at).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}</span>
            </div>
            {n.user_message && (
              <div style={{ fontSize: 11, color: "#1A1612", marginTop: 4, wordBreak: "break-word" }}>
                <span style={{ fontSize: 9, color: "#9A8E82", fontWeight: 600, marginRight: 6 }}>USR</span>
                « {n.user_message.slice(0, 220)}{n.user_message.length > 220 ? "…" : ""} »
              </div>
            )}
            {n.ai_response && (
              <div style={{ fontSize: 11, color: "#6B6358", marginTop: 4, fontStyle: "italic", wordBreak: "break-word" }}>
                <span style={{ fontSize: 9, color: "#9A8E82", fontWeight: 600, marginRight: 6, fontStyle: "normal" }}>IA</span>
