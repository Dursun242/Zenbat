...
export default function ClientDetail({ c, clientDevis, onBack, goDevis, onUpdate, onDelete }) {
  ...
  return (
    <div style={{ padding: 18 }} className="fu">
      <button onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "#64748b", fontSize: 13, marginBottom: 14, cursor: "pointer" }}>
        {I.back} Retour
      </button>
      ...
      <div style={{ background: "white", borderRadius: 14, border: "1px solid #f1f5f9", padding: 18, marginBottom: 12 }}>
        ...
        {fields.map(([k, v]) => (
          <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "8px 0", borderTop: "1px solid #f8fafc" }}>
            <span style={{ fontSize: 12, color: "#94a3b8", flexShrink: 0 }}>{k}</span>
            <span style={{ fontSize: 12, color: "#0f172a", fontWeight: 500, textAlign: "right", wordBreak: "break-word" }}>{v}</span>
          </div>
        ))}
      ...
      <div style={{ background: "white", borderRadius: 14, border: "1px solid #f1f5f9", overflow: "hidden" }}>
        <div style={{ padding: "12px 16px", borderBottom: "1px solid #f8fafc", fontWeight: 600, fontSize: 13, color: "#0f172a" }}>
          Devis ({clientDevis.length})
        </div>
        {clientDevis.length === 0 && (
          <div style={{ padding: "14px 16px", fontSize: 12, color: "#94a3b8" }}>Aucun devis pour ce contact</div>
        )}
        {clientDevis.map(d => (
          <div key={d.id} onClick={() => goDevis(d.id)}
            ...
            <div>
              ...
              <div style={{ fontSize: 10, fontFamily: "monospace", color: "#94a3b8", marginTop: 2 }}>{d.numero}</div>
            </div>
